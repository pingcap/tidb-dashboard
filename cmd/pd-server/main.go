...
var (
	...
	advertiseClientPort = flag.Uint64("advertise-client-port", 0, "advertise port for client traffic (default '{client-port}')")
	peerPort            = flag.Uint64("peer-port", 2380, "port for peer traffic")
	advertisePeerPort   = flag.Uint64("advertise-peer-port", 0, "advertise port for peer traffic (default '{peer-port}')")
	port                = flag.Uint64("port", 1234, "server port (deprecate later)")
	advertisePort       = flag.Uint64("advertise-port", 0, "advertise server port (deprecate later) (default '{port}')")
	httpPort            = flag.Uint64("http-port", 9090, "http port (deprecate later)")
	initialCluster      = flag.String("initial-cluster", "", "initial cluster configuration for bootstrapping (default '{name}=http://{host}:{advertise-peer-port}')")
