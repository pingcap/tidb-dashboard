...
func (t AvgMaxMinTableDef) queryRow(arg *queryArg, db *gorm.DB) (*TableRowDef, error) {
	...
	args := []interface{}{arg.startTime, arg.endTime}
	condition := "where time >= ? and time < ? "
...
func (t totalTimeByLabelsTableDef) genSumarySQLs(totalTime float64, startTime, endTime string, quantiles []float64) (string, []interface{}) {
	sqls := []string{
		fmt.Sprintf("select '%[1]s','', if(%[2]v>0,sum(value)/%[2]v,1) , sum(value) from metrics_schema.%[3]s_total_time where time >= ? and time < ?",
			t.name, totalTime, t.tbl),
		fmt.Sprintf("select sum(value) from metrics_schema.%s_total_count where time >= ? and time < ?",
			t.tbl),
	}
	args := []interface{}{startTime, endTime, startTime, endTime}
	for _, quantile := range quantiles {
		sql := fmt.Sprintf("select max(value) as max_value from metrics_schema.%s_duration where time >= ? and time < ? and quantile=?",
			t.tbl)
		sqls = append(sqls, sql)
		args = append(args, startTime, endTime, quantile)
	}
	fields := ""
	tbls := ""
...
func (t totalTimeByLabelsTableDef) genDetailSQLs(totalTime float64, startTime, endTime string, quantiles []float64) (string, []interface{}) {
	if len(t.labels) == 0 {
		return "", []interface{}{}
	}
	joinSQL := "select t0.*,t1.total_count"
	sqls := []string{
		fmt.Sprintf("select '%[1]s', `%[4]s`, if(%[2]v>0,sum(value)/%[2]v,1) , sum(value) as total from metrics_schema.%[3]s_total_time where time >= ? and time < ? group by `%[4]s` having sum(value) > 0",
			t.name, totalTime, t.tbl, strings.Join(t.labels, "`,`")),

		fmt.Sprintf("select `%[2]s`, sum(value) as total_count from metrics_schema.%[1]s_total_count where time >= ? and time < ? group by `%[2]s`",
			t.tbl, strings.Join(t.labels, "`,`")),
	}
	args := []interface{}{startTime, endTime, startTime, endTime}
	for i, quantile := range quantiles {
		sql := fmt.Sprintf("select `%[2]s`, max(value) as max_value from metrics_schema.%[1]s_duration where time >= ? and time < ? and quantile=? group by `%[2]s`",
			t.tbl, strings.Join(t.labels, "`,`"))
		sqls = append(sqls, sql)
		args = append(args, startTime, endTime, quantile)
...
func (t totalValueAndTotalCountTableDef) genSumarySQLs(startTime, endTime string, quantiles []float64) (string, []interface{}) {
	sqls := []string{
		fmt.Sprintf("select '%[1]s','' , sum(value) from metrics_schema.%[2]s where time >= ? and time < ?",
			t.name, t.sumTbl),
		fmt.Sprintf("select sum(value) from metrics_schema.%s where time >= ? and time < ?",
			t.countTbl),
	}
	args := []interface{}{startTime, endTime, startTime, endTime}
	for _, quantile := range quantiles {
		sql := fmt.Sprintf("select max(value) as max_value from metrics_schema.%s where time >= ? and time < ? and quantile=?",
			t.tbl)
		sqls = append(sqls, sql)
		args = append(args, startTime, endTime, quantile)
	}
...
func (t totalValueAndTotalCountTableDef) genDetailSQLs(startTime, endTime string, quantiles []float64) (string, []interface{}) {
	if len(t.labels) == 0 {
		return "", []interface{}{}
	}
	joinSQL := "select t0.*,t1.count"
	sqls := []string{
		fmt.Sprintf("select '%[1]s', `%[2]s` , sum(value) as total from metrics_schema.%[2]s where time >= ? and time < ? group by `%[2]s` having sum(value) > 0",
			t.name, t.sumTbl, strings.Join(t.labels, "`,`")),
		fmt.Sprintf("select `%[2]s`, sum(value) as count from metrics_schema.%[1]s where time >= ? and time < ? group by `%[2]s`",
			t.countTbl, strings.Join(t.labels, "`,`")),
	}
	args := []interface{}{startTime, endTime, startTime, endTime}
	for i, quantile := range quantiles {
		sql := fmt.Sprintf("select `%[2]s`, max(value) as max_value from metrics_schema.%[1]s where time >= ? and time < ? and quantile=? group by `%[2]s`",
			t.tbl, strings.Join(t.labels, "`,`"))
		sqls = append(sqls, sql)
		args = append(args, startTime, endTime, quantile)
		joinSQL += fmt.Sprintf(",t%v.max_value", i+2)
