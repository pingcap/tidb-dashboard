...
func GetClusterHardwareInfoTable(startTime, endTime string, db *sql.DB) (*TableDef, error) {
	...
	sql = "SELECT `INSTANCE`,`DEVICE_NAME`,`VALUE` from information_schema.CLUSTER_HARDWARE where `NAME` = 'total' AND `DEVICE_TYPE` = 'disk' AND (`DEVICE_NAME` LIKE '/dev%' or `DEVICE_NAME` LIKE 'sda%' or`DEVICE_NAME` LIKE 'nvme%')"
	rows, err = querySQL(db, sql)
	if err != nil {
		return nil, err
	}
