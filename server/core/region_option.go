...
// SetReplicateStatus sets the region's replicate status.
func SetReplicateStatus(status *replicate_mode.RegionReplicateStatus) RegionCreateOption {
	return func(region *RegionInfo) {
		region.replicateStatus = status
	}
}
