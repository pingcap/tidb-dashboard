...
const postCSSPlugin = ({
  ...
  setup(build) {
    ...
    build.onResolve({ filter: /.\.(css|sass|scss|less|styl)$/ }, async (args) => {
      ...
      if (!sourceFullPath) sourceFullPath = import_path.default.resolve(args.resolveDir, args.path)
      ...
      if (sourceExt === '.less')
        css = (
          await import_less.default.render(new import_util.TextDecoder().decode(fileContent), {
            ...lessOptions,
            filename: sourceFullPath,
