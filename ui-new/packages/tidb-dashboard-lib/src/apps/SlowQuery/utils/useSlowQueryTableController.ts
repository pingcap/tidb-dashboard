...
import { SlowqueryModel } from '../context/model'
...
export default function useSlowQueryTableController(
  ...
  useEffect(() => {
    async function querySchemas() {
      try {
        const res = await ctx?.ds.infoListDatabases({
          data: {
  ...
  useEffect(() => {
    ...
    async function getSlowQueryList() {
      ...
      try {
        const res = await ctx?.ds.slowQueryListGet(
  ...
  async function downloadCSV() {
    try {
      ...
      if (token) {
        window.location.href = `${ctx?.config.basePath}/slow_query/download?token=${token}`
      }
    } finally {
