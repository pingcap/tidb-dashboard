...
describe('SQL statements list page', () => {
  ...
  //           })
  //           .then(() => {
  //             cy.get('[data-item-key=sum_cop_task_num]')
  //               .should('have.text', 'Total Coprocessor Tasks')
  //           })
  //       })
  //   })
  ...
  describe('Statement Setting', () => {
    ...
    const getAriaValueNow = (dataE2EValue) => {
      cy.get(`[data-e2e=${dataE2EValue}]`).within(() => {
        cy.get('.ant-slider-handle').then(($el) => {
          console.log('value', $el[0].ariaValueNow)
          return $el[0].ariaValueNow
        })
      })
    }
    ...
    it('SQL Statement History Size', () => {
      cy.get('[data-e2e=statement_setting]')
        .click()
        .then(() => {
          const refreshInterval = getAriaValueNow(
            'statement_setting_refresh_interval'
          )
          const historySize = getAriaValueNow('statement_setting_history_size')
          console.log('refreshInterval', refreshInterval, historySize)
