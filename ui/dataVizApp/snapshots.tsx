...
export function downloadSnapshotsJSON(
...
export class SnapshotImportLocal extends React.Component<
  ImportProps,
  ImportState
> {
  ...
  readFile(e: React.ChangeEvent<HTMLInputElement>) {
    if (e.target.files) {
      ...
      reader.onload = () => {
        ...
        let snapshots: SandDance.types.Snapshot[] | undefined
        try {
          snapshots = JSON.parse(rawText)
        } catch (e) {
          this.setState({
            fileFormatError: strings.errorInvalidFileFormat,
            working: false,
          })
        }
        //validate these are snapshots
        if (snapshots && validSnapshots(snapshots)) {
...
export class SnapshotImportRemote extends React.Component<
  ImportRemoteProps,
  ImportRemoteState
> {
  ...
  render() {
    ...
    if (
      ...
      this.state.url &&
      !invalidUrlError(this.state.url) &&
      !this.state.urlError
    ) {
        ...
        >
          <section>
            <base.fluentUI.TextField
              ...
              onChange={(e, url) => this.setState({ url, urlError: '' })}
    ...
    >
      <ul>
        <li>
          ...
          <div>{strings.labelSnapshotsExportHTMLDescription}</div>
          <base.fluentUI.PrimaryButton
            ...
            onClick={(e) => {
              ...
              const html = getEmbedHTML(
                props.explorer.state.dataContent.data,
                props.dataSource.displayName || 'data',
                clean
              )
              downloadData(
                html,
                `${props.dataSource.displayName || 'data'}.html`
              )
            }}
          />
        </li>
        <li>
          ...
          <div>{strings.labelSnapshotsExportMarkdownDescription}</div>
          <base.fluentUI.PrimaryButton
            iconProps={{ iconName: 'Download' }}
            ...
            onClick={(e) => {
              const sections = props.snapshots.map((snapshot) => {
                ...
                section.push(
                  markdownImageLink(snapshot.title!, snapshot.image!, url)
                )
                return section.join('\n')
              })
