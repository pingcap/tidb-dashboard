...
import { Form, Select, Button, Space, Modal, Checkbox } from 'antd'
...
export default function ExportTablesPage() {
  ...
  const onFinish = async (f) => {
    ...
    //   })
    //   Modal.success({
    //     title: t('data_manager.dump_tables.export_success'),
    //     onOk: () => {
    ...
    //       window.open(url)
    //     },
    //     okText: t('data_manager.dump_tables.export_download'),
    //   })
    // } catch (e) {
    //   Modal.error({
    //     title: t('data_manager.dump_tables.export_failed'),
  ...
  return (
    <>
      <Head
        title={t('data_manager.dump_tables.title')}
        back={
      ...
      <Card>
        ...
        >
          <Form.Item
            label={t('data_manager.dump_tables.form.tables')}
            name="tables"
            rules={[{ required: true }]}
          >
            <Select
              mode="multiple"
              placeholder={t(
                'data_manager.dump_tables.form.tables_placeholder'
              )}
              allowClear
              disabled={isLoading}
            >
          ...
          <Form.Item
            name="includes"
            label={t('data_manager.dump_tables.form.no_data')}
            rules={[{ required: true }]}
          >
            <Select>
              <Select.Option value="data">
                {t('data_manager.dump_tables.form.no_data_opt.data')}
              </Select.Option>
              <Select.Option value="no_data">
                {t('data_manager.dump_tables.form.no_data_opt.no_data')}
              </Select.Option>
            </Select>
          </Form.Item>
          <Form.Item
            name="insert_kind"
            label={t('data_manager.dump_tables.form.insert_kind')}
            rules={[{ required: true }]}
          >
            <Select>
              <Select.Option value="insert">INSERT</Select.Option>
              <Select.Option value="insert ignore">INSERT IGNORE</Select.Option>
          ...
          <Form.Item>
            <Space>
              <Button disabled={isLoading} onClick={() => selectAll()}>
                {t('data_manager.dump_tables.form.select_all')}
              </Button>
              <Button type="primary" htmlType="submit" loading={isLoading}>
                {t('data_manager.dump_tables.form.submit')}
              </Button>
            </Space>
          </Form.Item>
        </Form>
