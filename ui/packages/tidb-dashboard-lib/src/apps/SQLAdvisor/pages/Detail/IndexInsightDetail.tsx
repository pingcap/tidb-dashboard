...
import { TunedDetailData } from './indexInsightData'

console.log('TunedDetailData', TunedDetailData)
  ...
  useEffect(() => {
    const sqlTunedDetailGet = async () => {
      try {
        ...
        setSqlTunedDetail(TunedDetailData!)
      } catch (e) {
      } finally {
        setLoading(false)
      }
  ...
  return (
    <div>
      <Head
        title={
          sqlTunedDetail
            ? `Index Insight Detail - ${sqlTunedDetail.insight_type}`
            : 'Index Insight Detail'
        }
        back={
          <Link to="/sql_advisor">
            <ArrowLeftOutlined />
