...
export default function SearchProgress({
  ...
  async function getTasks(taskGroupID: number, tasks: LogsearchTaskModel[]) {
    ...
    const res = await client.getGlobal().logsTaskgroupsIdGet(taskGroupID)
  ...
  async function handleDownload() {
    ...
    const res = await client.getGlobal().logsDownloadAcquireTokenGet(keys)
    const token = res.data
    if (!token) {
      return
    }
    const url = `${client.getGlobal()['basePath']}/logs/download?token=${token}`
    window.open(url)
  }
  ...
  async function handleCancel() {
    ...
    confirm({
      ...
      onOk() {
        client.getGlobal().logsTaskgroupsIdCancelPost(taskGroupID)
        setTasks(
          tasks.map(task => {
  ...
  async function handleRetry() {
    ...
    confirm({
      ...
      onOk() {
        client.getGlobal().logsTaskgroupsIdRetryPost(taskGroupID)
        setTasks(
          tasks.map(task => {
            if (task.state === TaskState.Error) {
              task.state = TaskState.Running
